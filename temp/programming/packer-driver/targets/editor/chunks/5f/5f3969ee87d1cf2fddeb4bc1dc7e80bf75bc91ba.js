    ...
    execute: function () {
      ...
      _export("Match3Game", Match3Game = (_dec = ccclass('Match3Game'), _dec2 = property(Node), _dec3 = property(Node), _dec(_class = (_class2 = class Match3Game extends Component {
        ...
        onTouchMove(event) {
          ...
          const dy = localPos.y - this.touchStartPos.y; // 滑动距离足够大时触发交换
        ...
        findMatches() {
          ...
          for (let row = 0; row < this.gridSize - 2; row++) {
            for (let col = 0; col < this.gridSize; col++) {
              if (this.gems[row][col] === this.gems[row + 1][col] && this.gems[row][col] === this.gems[row + 2][col]) {
                matches.push({
                  row,
                  col,
                  direction: 'vertical'
        ...
        updateUI() {
          ...
          if (this.uiRoot) {
            ...
            const progress = Math.min(this.score / this.target, 1); // 进度条动画...
          }
        }
