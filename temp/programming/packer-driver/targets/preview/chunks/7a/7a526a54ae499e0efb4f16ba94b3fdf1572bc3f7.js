  ...
  return {
    ...
    execute: function () {
      ...
      _export("TetrisGame", TetrisGame = (_dec = ccclass('TetrisGame'), _dec2 = property(Node), _dec3 = property(Node), _dec(_class = (_class2 = class TetrisGame extends Component {
        ...
        spawnPiece() {
          var piece = this.pieces[Math.floor(Math.random() * this.pieces.length)];
          this.currentPiece = {
        ...
        lockPiece() {
          for (var i = 0; i < this.currentPiece.shape.length; i++) {
            for (var j = 0; j < this.currentPiece.shape[i].length; j++) {
              if (this.currentPiece.shape[i][j]) {
                var y = this.currentPiece.y + i;
                var x = this.currentPiece.x + j;
        ...
        rotatePiece() {
          var rotated = this.currentPiece.shape[0].map((_, i) => this.currentPiece.shape.map(row => row[i]).reverse());
        ...
        render() {
          ...
          for (var i = 0; i < this.rows; i++) {
            for (var j = 0; j < this.cols; j++) {
              if (this.board[i][j] !== -1) {
                var node = this.createCell(this.colors[this.board[i][j]], j, i);
          ...
          if (this.currentPiece) {
            for (var _i = 0; _i < this.currentPiece.shape.length; _i++) {
              for (var _j = 0; _j < this.currentPiece.shape[_i].length; _j++) {
                if (this.currentPiece.shape[_i][_j]) {
                  var _node = this.createCell(this.colors[this.currentPiece.color], this.currentPiece.x + _j, this.currentPiece.y + _i);

                  this.gameArea.addChild(_node);
                }
              }
            }
          }
